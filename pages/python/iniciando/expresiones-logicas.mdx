...
|:---|:---|
|`3==4`|¿`3` es igual a `4`?<br/>Ten en cuenta que el doble signo igual es de comparación y no de asignación.|
...
El operador `and` obtiene dos expresiones lógicas como operandos y se evalúa como `True` si y sólo si tanto la primera como la segunda expresión se han evaluado como `True`. Corresponde pues a la palabra `and` en el lenguaje natural que une dos partes de una oración. En el ejemplo, esto podría verse así:
...
El operador `or` obtiene dos expresiones lógicas como operandos y se evalúa como `False` solo si tanto la primera expresión como la segunda se evalúan como `False`. Así, el operador devuelve `True` si al menos uno de sus operandos ha devuelto `True`:
